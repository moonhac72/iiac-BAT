...
PATROL_BASE = r"c:\Users\moonh\Documents\iiac-BAT\10_Wiki\🛠️ Projects\야통대_자료\2023년도 야통대\일일점검"

WEEKDAY_KR = ["월", "화", "수", "목", "금", "토", "일"]

SEASON_ENV = {
    1: ("맑음", -6.5, 1.2, "북서풍(NW)", 8.5, "겨울철 혹한기"),
    2: ("맑음", -3.2, 4.5, "북서풍(NW)", 7.8, "늦겨울 해빙기"),
    3: ("구름조금", 2.1, 10.8, "서풍(W)", 8.2, "초봄 해빙기"),
    4: ("맑음", 7.5, 16.2, "남서풍(SW)", 7.1, "봄철 조류 번식기"),
    5: ("맑음", 13.2, 22.4, "남서풍(SW)", 6.5, "봄철 번식 절정기"),
    6: ("흐림", 18.5, 26.8, "남서풍(SW)", 6.8, "초여름 장마기"),
    7: ("구름많음", 23.4, 29.5, "남풍(S)", 5.9, "여름 혹서기"),
    8: ("맑음", 24.1, 30.2, "남동풍(SE)", 6.2, "늦여름 태풍기"),
    9: ("맑음", 18.2, 25.4, "북서풍(NW)", 6.9, "초가을 철새 남하기"),
    10: ("맑음", 11.5, 19.8, "북서풍(NW)", 7.8, "가을철 이동 절정기"),
    11: ("구름조금", 4.2, 12.1, "북서풍(NW)", 8.4, "초겨울 월동 정착기"),
    12: ("맑음", -4.8, 2.5, "북서풍(NW)", 9.1, "한겨울 동절기 혹한기")
}

def clean_old_monthly_summaries():
    print(">>> Cleaning old 2-file monthly summaries in 일일점검...")
    for m in range(1, 13):
        m_dir = os.path.join(PATROL_BASE, f"{m}월 일일점검")
        if os.path.exists(m_dir):
            for f in os.listdir(m_dir):
                if f.endswith("통합보고서.md") or f.endswith("브리핑_및_인사이트.md"):
                    os.remove(os.path.join(m_dir, f))
    print("Old monthly summaries cleaned.")

def generate_daily_files():
    df = pd.read_csv(CSV_PATROL)
    dates = sorted(df['Date'].dropna().unique())
    
    print(f">>> Generating {len(dates)} daily files for 2023...")
    ...
    for d_str in dates:
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write(f"- [[맞바람 착륙 풍향과 연계한 활주로 남단·북단 조류 감시 SOP]]\n")
            f.write(f"- [[기러기류의 일출·일몰 전후 이동 패턴과 야간 휴식지 집중 순찰 기법]]\n")
            f.write(f"- [[야간·조간 배수로 사각지대 확인 사격 기준 및 공포탄 화재 리스크 관리]]\n\n")
    ...
    print(f"Daily files generation complete: {len(dates)} files written.")

if __name__ == '__main__':
    clean_old_monthly_summaries()
