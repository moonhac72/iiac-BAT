...
FILE_PATROL_2022 = r"C:\Users\moonh\Desktop\Anythingllm 자료폴더\조류 통계\일일점검(22년).xlsx"
...
OUT_PATROL_CSV = r"c:\Users\moonh\Documents\iiac-BAT\_data_raw\resampled\2022_total_resampled.csv"
OUT_STRIKE_CSV = r"c:\Users\moonh\Documents\iiac-BAT\_data_raw\resampled\2022_birdstrike_resampled.csv"
OUT_CARCASS_CSV = r"c:\Users\moonh\Documents\iiac-BAT\_data_raw\resampled\2022_carcass_resampled.csv"
...
def resample_patrol_2022():
    print(">>> Resampling 2022 Patrol Data...")
    wb = openpyxl.load_workbook(FILE_PATROL_2022, data_only=True)
    ws = wb['야생동물 점검(개별조회)']
    ...
    print(f"Patrol 2022 Done: {len(df)} rows saved to {OUT_PATROL_CSV}")
    return df

def resample_strike_2022():
    print(">>> Resampling 2022 Bird Strikes...")
    ...
    print(f"Strikes 2022 Done: {len(df)} rows saved to {OUT_STRIKE_CSV}")
    return df

def resample_carcass_2022():
    print(">>> Resampling 2022 Carcass Data...")
    ...
    print(f"Carcass 2022 Done: {len(df)} rows saved to {OUT_CARCASS_CSV}")
    return df

def main():
    p_df = resample_patrol_2022()
    s_df = resample_strike_2022()
    c_df = resample_carcass_2022()
    
    print("\n==========================================")
    print("=== 2022 YATONGDAE RESAMPLING SUMMARY ===")
    print("==========================================")
    ...
    print(f"\nTotal Bird Strikes (2022): {len(s_df)} (Damaged: {len(s_df[s_df['Is_Damage']==1])})")
    print(f"Total Carcasses (2022): {len(c_df)} (Strike Probable: {len(c_df[c_df['Is_Strike_Probable']==1])})")
