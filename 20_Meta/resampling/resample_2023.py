...
FILE_PATROL_2023 = r"C:\Users\moonh\Desktop\Anythingllm 자료폴더\조류 통계\일일점검(23년).xlsx"
...
OUT_PATROL_CSV = r"c:\Users\moonh\Documents\iiac-BAT\_data_raw\resampled\2023_total_resampled.csv"
OUT_STRIKE_CSV = r"c:\Users\moonh\Documents\iiac-BAT\_data_raw\resampled\2023_birdstrike_resampled.csv"
OUT_CARCASS_CSV = r"c:\Users\moonh\Documents\iiac-BAT\_data_raw\resampled\2023_carcass_resampled.csv"
...
def determine_sun_window(month, hour):
    # Simplified astronomical window for Incheon Airport
    # Winter (11, 12, 1, 2): Sunrise ~07:30, Sunset ~17:30
    # Spring/Autumn (3, 4, 9, 10): Sunrise ~06:30, Sunset ~18:30
    # Summer (5, 6, 7, 8): Sunrise ~05:30, Sunset ~19:30
...
def resample_patrol_2023():
    print(">>> Resampling 2023 Patrol Data...")
    wb = openpyxl.load_workbook(FILE_PATROL_2023, data_only=True)
    ws = wb['야생동물 점검(개별조회)']
    
    records = []
    # Row 3 is header: [None, 'NO.', '점검일자', '시간', '조류종', '개체수', '활동', '장비', '점검자', '탄약사용자', '7,1/2', 4, '공포탄', '대분류', '중분류', '지역', '지역상세', '맵영역그룹', '지표제외']
    ...
    print(f"Patrol 2023 Done: {len(df)} rows saved to {OUT_PATROL_CSV}")
    return df

def resample_strike_2023():
    print(">>> Resampling 2023 Bird Strikes...")
    wb = openpyxl.load_workbook(FILE_STRIKE_21_23, data_only=True)
    ws = wb['목록']
    
    records = []
    # Row 3 is header: ['일자', '시간', '조류종', '소속', '편명', '기종', '노선', '활주로', '고도', '운항단계', '충돌부위', '피해여부']
    for r in range(4, ws.max_row + 1):
        d_val = ws.cell(row=r, column=3).value
        if not d_val:
            continue
        d_str = str(d_val)[:10]
        if not d_str.startswith('2023'):
            continue
    ...
    print(f"Strikes 2023 Done: {len(df)} rows saved to {OUT_STRIKE_CSV}")
    return df

def resample_carcass_2023():
    print(">>> Resampling 2023 Carcass Data...")
    wb = openpyxl.load_workbook(FILE_CARCASS_21_23, data_only=True)
    ws = wb['기동지역사체 수거관리']
    
    records = []
    # Row 3 is header: ['NO', '일자', '시간', '수거위치', '조류명', '날씨', '상태', '비고']
    for r in range(4, ws.max_row + 1):
    ...
    print(f"Carcass 2023 Done: {len(df)} rows saved to {OUT_CARCASS_CSV}")
    return df

def main():
    p_df = resample_patrol_2023()
    s_df = resample_strike_2023()
    c_df = resample_carcass_2023()
    
    print("\n==========================================")
    print("=== 2023 YATONGDAE RESAMPLING SUMMARY ===")
    print("==========================================")
    ...
    print(f"\nTotal Bird Strikes (2023): {len(s_df)} (Damaged: {len(s_df[s_df['Is_Damage']==1])})")
    print(f"Total Carcasses (2023): {len(c_df)} (Strike Probable: {len(c_df[c_df['Is_Strike_Probable']==1])})")
