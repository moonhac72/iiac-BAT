...
FILE_EXCEL_2017 = r"C:\Users\moonh\Desktop\일일 17~20\일일2017.xlsx"
OUT_CSV_2017 = r"c:\Users\moonh\Documents\iiac-BAT\_data_raw\resampled\2017_total_resampled.csv"
PATROL_BASE = r"c:\Users\moonh\Documents\iiac-BAT\10_Wiki\🛠️ Projects\야통대_자료\2017년도 야통대\일일점검"
...
def load_2017_excel():
    print(f">>> Reading Excel: {FILE_EXCEL_2017} ...")
    wb = openpyxl.load_workbook(FILE_EXCEL_2017, read_only=True)
    ws = wb[wb.sheetnames[0]]
    
    records = []
    # Header: NO., 점검일자(2), 시간(3), 조류종(4), 개체수(5), 활동(6), 장비(7), 점검자(8), 탄약사용자(9), 7,1/2(10), 4(11), 공포탄(12), 대분류(13), 중분류(14), 지역(15), 지역상세(16), 맵영역그룹(17), 지표제외(18)
    ...
    print(f">>> Loaded {len(df)} records from 2017 Excel.")
    
    # Save Resampled CSV for persistence
    os.makedirs(os.path.dirname(OUT_CSV_2017), exist_ok=True)
    df.to_csv(OUT_CSV_2017, index=False, encoding='utf-8-sig')
    print(f">>> Saved resampled CSV: {OUT_CSV_2017}")
    return df

def generate_daily_files_2017(df):
    dates = sorted(df['Date'].dropna().unique())
    print(f">>> Generating {len(dates)} daily files for 2017 in {PATROL_BASE} ...")
    ...
    for d_str in dates:
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write(f"태그: #야통대 #일일점검 #항공안전 #인천공항 #{season_desc.replace(' ', '')} #큰기러기 #7호탄 #4호탄 #풍향분석 #야간순찰 #2017년기록\n\n")
    ...
    print(f">>> 2017 Daily files generation complete: {len(dates)} files written successfully.")

if __name__ == '__main__':
    df = load_2017_excel()
    generate_daily_files_2017(df)
