...
FILE_EXCEL_2018 = r"C:\Users\moonh\Desktop\일일 17~20\일일2018.xlsx"
OUT_CSV_2018 = r"c:\Users\moonh\Documents\iiac-BAT\_data_raw\resampled\2018_total_resampled.csv"
PATROL_BASE = r"c:\Users\moonh\Documents\iiac-BAT\10_Wiki\🛠️ Projects\야통대_자료\2018년도 야통대\일일점검"
...
def load_2018_excel():
    print(f">>> Reading Excel: {FILE_EXCEL_2018} ...")
    wb = openpyxl.load_workbook(FILE_EXCEL_2018, read_only=True)
    ...
    print(f">>> Loaded {len(df)} records from 2018 Excel.")
    
    # Save Resampled CSV for persistence
    os.makedirs(os.path.dirname(OUT_CSV_2018), exist_ok=True)
    df.to_csv(OUT_CSV_2018, index=False, encoding='utf-8-sig')
    print(f">>> Saved resampled CSV: {OUT_CSV_2018}")
    return df

def generate_daily_files_2018(df):
    dates = sorted(df['Date'].dropna().unique())
    print(f">>> Generating {len(dates)} daily files for 2018 in {PATROL_BASE} ...")
    ...
    for d_str in dates:
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write(f"태그: #야통대 #일일점검 #항공안전 #인천공항 #{season_desc.replace(' ', '')} #큰기러기 #7호탄 #4호탄 #풍향분석 #야간순찰 #2018년기록\n\n")
    ...
    print(f">>> 2018 Daily files generation complete: {len(dates)} files written successfully.")

if __name__ == '__main__':
    df = load_2018_excel()
    generate_daily_files_2018(df)
