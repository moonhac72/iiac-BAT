...
FILE_EXCEL_2020 = r"C:\Users\moonh\Desktop\일일 17~20\일일2020.xlsx"
OUT_CSV_2020 = r"c:\Users\moonh\Documents\iiac-BAT\_data_raw\resampled\2020_total_resampled.csv"
PATROL_BASE = r"c:\Users\moonh\Documents\iiac-BAT\10_Wiki\🛠️ Projects\야통대_자료\2020년도 야통대\일일점검"
...
def load_2020_excel():
    print(f">>> Reading Excel: {FILE_EXCEL_2020} ...")
    wb = openpyxl.load_workbook(FILE_EXCEL_2020, read_only=True)
    ...
    print(f">>> Loaded {len(df)} records from 2020 Excel.")
    
    # Save Resampled CSV for persistence
    os.makedirs(os.path.dirname(OUT_CSV_2020), exist_ok=True)
    df.to_csv(OUT_CSV_2020, index=False, encoding='utf-8-sig')
    print(f">>> Saved resampled CSV: {OUT_CSV_2020}")
    return df

def generate_daily_files_2020(df):
    dates = sorted(df['Date'].dropna().unique())
    print(f">>> Generating {len(dates)} daily files for 2020 in {PATROL_BASE} ...")
    ...
    for d_str in dates:
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write(f"태그: #야통대 #일일점검 #항공안전 #인천공항 #{season_desc.replace(' ', '')} #큰기러기 #7호탄 #4호탄 #풍향분석 #야간순찰 #2020년기록\n\n")
    ...
    print(f">>> 2020 Daily files generation complete: {len(dates)} files written successfully.")

if __name__ == '__main__':
    df = load_2020_excel()
    generate_daily_files_2020(df)
